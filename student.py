...
from pytorch_lightning.loggers import TensorBoardLogger
from pytorch_lightning.callbacks import ModelCheckpoint
from teacher import Inception4_teacher_Model
import torch.nn.functional as F


class Inception4_Model(pl.LightningModule):
    def __init__(self,
                 model_type,
                 num_classes,
                 classes_weights,
        ...
        self.T = T
        self.alpha = alpha
        self.teacher_model = Inception4_teacher_Model.load_from_checkpoint("/data/ppr/distillation_Resnet18_Google_landmark_recog/weights/inception_v4__epoch=10_val_loss=2.400_val_acc=0.611_val_f1_epoch=0.000.ckpt").eval()
        # load nn
    ...
    def loss2(self, output_S, output_T):
        return self.loss_func2(input=output_S, target=output_T)*self.T*self.T
    ...
    def training_step(self, batch, batch_idx):
        ...
        soft_target = self.teacher_model(x)
        train_loss1 = self.loss(output, y)
        output_S = F.log_softmax(output/self.T, dim=1)
        output_T = F.softmax(soft_target/self.T, dim=1)

        train_loss2 = self.loss2(output_S, output_T)
        train_loss = train_loss1 * (1-self.alpha) + train_loss2 * self.alpha
    ...
    def validation_step(self, batch, batch_idx):
        ...
        soft_target = self.teacher_model(x)
        val_loss1 = self.loss(output, y)
        output_S = F.log_softmax(output / self.T, dim=1)
        output_T = F.softmax(soft_target / self.T, dim=1)

        val_loss2 = self.loss2(output_S, output_T)
        val_loss = val_loss1 * (1 - self.alpha) + val_loss2 * self.alpha
    ...
    def test_step(self, batch, batch_idx):
        ...
        soft_target = self.teacher_model(x)
        test_loss1 = self.loss(output, y)
        output_S = F.log_softmax(output / self.T, dim=1)
        output_T = F.softmax(soft_target / self.T, dim=1)

        test_loss2 = self.loss2(output_S, output_T)
        test_loss = test_loss1 * (1 - self.alpha) + test_loss2 * self.alpha
...
def train():
    ...
    for m in tqdm(models, disable=True):
        ...
        # Init our model
        model = Inception4_Model(model_type=model_data['model']['model_type'],
        ...
        # trainer = pl.Trainer(max_epcochs=max_epochs, 
        #                     progress_bar_refresh_rate=progress_bar_refresh_rate,
